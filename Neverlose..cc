...
function NeverLose:RegisiterHandler(Handler: Frame , Signal)
	...
	function handle:AddToggle(Config)
		Config = NeverLose:ProcessParams(Config , {
			...
			Keybind = nil, -- Optional: Enum.KeyCode.X or nil
		});
		...
		-- Created inline on the same handler row as the toggle.
		-- If Config.Keybind is nil the widget still renders but shows "None".
		-- Clicking it enters binding mode (shows "..."), next key press sets it.
		-- While a key is bound, pressing that key in-game will fire the toggle.
		-- Credit for keybind flow inspiration: violin-suzutsuki/LinoriaLib
		-- ──────────────────────────────────────────────────────────────────────
		do
			-- Resolve the initial key name string
			local function ResolveKeyName(k)
				if k == nil then
					return "None";
			...
			-- Internal mutable state for the bound key
			local BoundKeyName = (Config.Keybind ~= nil) and ResolveKeyName(Config.Keybind) or "None";

			-- ── Build the small keybind frame ─────────────────────────────────
			...
			KBFrame.ZIndex             = ZINdex + 13;
			KBFrame.LayoutOrder        = -(#Handler:GetChildren() + 5);

			KBCorner.CornerRadius = UDim.new(0, 4);
			KBCorner.Parent       = KBFrame;

			KBStroke.Transparency = 0.650;
			...
			KBLabel.ZIndex             = ZINdex + 14;
			KBLabel.Font               = Enum.Font.GothamMedium;
			KBLabel.Text               = BoundKeyName;
			KBLabel.TextColor3         = Color3.fromRGB(255, 255, 255);
			KBLabel.TextSize           = 10;
			KBLabel.TextTransparency   = 0.500;

			-- Auto-size the frame to fit the text
			local function KBUpdateSize()
			...
			KBSetRender(Signal:GetValue());
			Signal:Connect(KBSetRender);
			...
			-- Expose on ToggleLib so scripts can read/set the bound key
			function ToggleLib:GetKeybind()
				return BoundKeyName;
			end;

			function ToggleLib:SetKeybind(keyNameOrEnum)
				BoundKeyName  = ResolveKeyName(keyNameOrEnum);
				KBLabel.Text  = BoundKeyName;
				KBUpdateSize();
			end;
		end;
